...
// 	pushfyi.init();//hb_user_supportuser_channel
	...
	if(event.keyCode == 13 && event.shiftKey == 0)  {
		...
		if (message != '') {
		    ...
		    userConfig.sendChatMessage(userConfig.private_channel_name,message, function(data){
				...
				$("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxmessage"><span class="chatboxmessagecontent">me: '+message+'</span></div>');
				$("#chatbox_"+chatboxtitle+" .chatboxcontent").scrollTop($("#chatbox_"+chatboxtitle+" .chatboxcontent")[0].scrollHeight);
				//userConfig
				if (userConfig.noSupportUserAssigned){
					//when user wants help
					userConfig.first_query_text = message;
					actions('JustArrived','NU',function(){});
					$(chatboxtextarea).val('getting you a support officer assigned.pelase wait.');
					$(".chatboxtextarea").attr('disabled','disabled');
				}
				else
					$(".chatboxtextarea").removeAttr('disabled');
			});
			//$.post("chat.php?action=sendchat", {to: chatboxtitle, message: message} ,);
		}
		chatHeartbeatTime = minChatHeartbeat;
		chatHeartbeatCount = 1;
